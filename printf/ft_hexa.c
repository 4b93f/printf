...
/*   Updated: 2020/03/06 14:13:08 by chly-huc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		apply(char *str, int *nb_space, t_flag *flag, int nb0)
{
	...
	while (nb0-- > 0)
		i += write(1, "0", 1);
	if (!(flag->v_p == 1 && flag->preci == 0 && !ft_strcmp(str, "0")))
...
int				ft_hexa(va_list args, t_flag *flag, int i)
{
	int			to_print;
	int			nb_space;
	char		*str;
	...
	if (!(str = ft_itoll(va_arg(args, unsigned int))))
		return (-1);
	if (!(str = ft_convert_base(str, NUMBER, BASE1)))
		return (-1);
	str = !str ? "(null)" : str;
